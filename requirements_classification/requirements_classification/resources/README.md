...
The databricks CLI bundles top level is defined by file `requirements_classification/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`requirements_classification/resources/model-workflow-resource.yml`)
 - batch inference workflow (`requirements_classification/resources/batch-inference-workflow-resource.yml`)
 - monitoring resource and workflow (`requirements_classification/resources/monitoring-resource.yml`)
 - feature engineering workflow (`requirements_classification/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`requirements_classification/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `requirements_classification/databricks.yml` and `requirements_classification/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-requirements_classification-model     | /dev-requirements_classification-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-requirements_classification-model | /staging-requirements_classification-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-requirements_classification-model    | /prod-requirements_classification-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-requirements_classification-model    | /test-requirements_classification-experiment    |
...
2. Complete the `TODO` in `requirements_classification/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `requirements_classification/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `requirements_classification/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-requirements_classification-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-requirements_classification-batch-inference-job`
that runs the notebook under `requirements_classification/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-requirements_classification-batch-inference-job`.
...
`test-requirements_classification-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-requirements_classification-batch-inference-job`.
